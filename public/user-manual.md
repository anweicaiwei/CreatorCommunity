...
- **本地开发**：运行 `npm run dev`，浏览器访问 `http://localhost:5173`
- **生产环境**：访问已部署的网站地址
...
**功能说明**：对已有帖子进行评论，评论者和帖子作者双方均可获得 CTK 奖励，基础各 0.1 CTK/次。
...
3. MetaMask 弹出交易确认窗口
4. 确认交易，等待上链
5. 交易成功后，评论者和作者各获得对应的 CTK 奖励（记录为"待领取"）
...
- **功能**：提取 NFT 合约中超出动态阈值的 CTK 余额
- **操作**：点击"提取溢出 CTK"
- **分配**：提取的 CTK 按 70% 创作者池 + 30% 互动池比例返还给 CreatorToken 合约
...
1. 页面中的 **"交易历史"** 区域会自动记录您在本应用中的所有链上操作
2. 每条记录包含：
   - 交易哈希（`txHash`）
   - 交易状态（成功 / 失败 / 处理中）
   - 操作时间
   - 关联的区块高度
3. 点击交易哈希可跳转至区块浏览器详情页，查看完整的链上交易信息
...
```
缓存键 = chainId + token合约地址 + 钱包地址
```

- **链上只读数据**：余额、冷却时间、NFT 信息等通过 `useDataCache` 缓存
- **帖子列表**：通过 `usePostList` 按链 + 合约 + 账户独立缓存
- **交易历史**：通过 `useTxHistory` 持久化到 localStorage

#### 缓存刷新策略

| 数据类型 | 刷新方式 |
|---------|---------|
| 余额、冷却时间等高频数据 | 交易完成后自动刷新 |
| 池统计等低频数据 | 定时刷新 |
| 帖子列表 | 发帖操作后刷新 |
| 交易历史 | 每次新交易追加 |
...
缓存数据按 `chainId + tokenAddress + accountAddress` 作为唯一键，确保：
- 切换网络后不会加载错误的数据
- 切换钱包后数据自动隔离
- 同一网络下不同合约地址的数据互不干扰
